...
# El modelo se crea con distintos valores para el hiperparametro de regularizacion
...
parameters = {'alpha':[10**(-3), 10**(-2), 10**(-1), 1, 10**1, 10**2, 10**3, 10**4, 10**5, 10**6]}

gridSearchCV = GridSearchCV(estimator=ridge_model, param_grid=parameters, scoring='neg_mean_absolute_error')
gridSearchCV.fit(X_train, Y_train)
print(gridSearchCV.cv_results_)
print(gridSearchCV.best_params_)

print("El valor del param de regularizacion que le estoy pasando es: " + str(gridSearchCV.best_params_['alpha']))

Y_test_predict = gridSearchCV.predict(X_test)
